        ...
        while (1)
        {
            ...
            if (CheckNewKeyFrames() && !mbBadImu)
            {
                ...
                if (!CheckNewKeyFrames() && !stopRequested())
                {
                    ...
                    float dist_dz = 0.0;
                    if (mpAtlas->KeyFramesInMap() > 2 && mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        dist_dz = (mpCurrentKeyFrame->mPrevKF->GetCameraCenter() -
